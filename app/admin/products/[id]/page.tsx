...
export default function EditProductPage() {
      ...
      try {
        ...
        if (product) {
          setForm({
            ...
            variants_json: product.variants.length > 0 ? JSON.stringify(product.variants, null, 2) : '',
          });
        }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <h1 className="font-mono text-[0.75rem] tracking-[0.15em] uppercase text-text-mid">Edit Product</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div><label className={labelClass}>Slug</label><input value={form.slug} onChange={(e) => update('slug', e.target.value)} className={inputClass} required /></div>
        ...
        <div><label className={labelClass}>Variants JSON</label><textarea value={form.variants_json} onChange={(e) => update('variants_json', e.target.value)} rows={4} className={inputClass} /></div>
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={saving} className="font-mono text-[0.75rem] tracking-[0.1em] uppercase bg-accent text-bg px-6 py-2.5 rounded-[4px] hover:bg-white transition-all disabled:opacity-50">{saving ? 'Saving...' : 'Save Changes'}</button>
